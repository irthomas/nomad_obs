...
for cop_row_name, dictionary_data in cop_summary_dict.items():
    ...
    if not os.path.exists(summary_row_path):
        ...
        for row_number in range(len(cop_row_data)):
            ...
            elif cell_colour_info.type == "indexed":
                rgb_or_theme = "indexed"
                color_theme = Sheet1.cell(row_number + 1, dictionary_data["empty_column"] - 1).fill.start_color.index
            else:
                rgb_or_theme = "theme"
