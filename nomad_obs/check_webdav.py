...
CHECK webdav FOR NEW DOCUMENTS
IF COP ROWS FOUND, EMAIL A SUMMARY


CHECK EVERY X MINUTES

IF COP TABLES ARE UPDATED, NEED TO UPDATE VERSION HERE
...
from nomad_obs.webdav.tools import WEBDAV_DATA_PATH, get_webdav_file_dict, download_file, get_orbit_plan_path, email_summary
# from nomad_obs.webdav.tools_ftp_old import FTP_PATH, get_ftp_file_dict, download_file, get_orbit_plan_path, email_summary
...
# get all files from webdav even if they exist?
...
def get_new_updated_webdav_file_dict():
    """get dictionary of new or updated files"""

    # check webdav for all files
    if VERBOSE:
        print("### Getting webdav file info dict for path:", WEBDAV_DATA_PATH)
        # print("### Getting webdav file info dict for path:", FTP_PATH)
    webdav_file_dict = get_webdav_file_dict()
    # webdav_file_dict = get_ftp_file_dict()

    new_files = {}

    # for each file on webdav, check if it also exists locally
    if VERBOSE:
        print("### Checking if files exist locally")
    for webdav_filepath in webdav_file_dict.keys():
        if VERBOSE:
        ...
        if webdav_file_dict[webdav_filepath]["type"] in ["cop_rows", "orbit_plans"]:
            ...
            if VERBOSE:
                print("Path found of type:", webdav_file_dict[webdav_filepath]["type"])
            ...
            if os.path.exists(local_filepath):
                ...
                if VERBOSE:
                    print("Matching local file found, setting to True:", local_filepath)

            else:
                # if it doesn't exist, add to dictionary
                webdav_file_dict[webdav_filepath]["exists"] = False
                if VERBOSE:
                    print("Matching local file not found, setting to False:", local_filepath)
    ...
    for webdav_filepath in webdav_file_dict.keys():
        ...
        if webdav_file_dict[webdav_filepath]["type"] in ["cop_rows", "orbit_plans"]:
            ...
            if not webdav_file_dict[webdav_filepath]["exists"] or FORCE_REDOWNLOAD:
                local_filepath = webdav_file_dict[webdav_filepath]["local_path"]
                local_filename = os.path.basename(local_filepath)
                if VERBOSE:
                    print("Matching local file not found, getting local path:", local_filepath)
                print("File %s doesn't exist locally - downloading from webdav" % local_filepath)
                ...
                # download
                if VERBOSE:
                    print("Downloading file from:", webdav_filepath, "to", local_filepath)
                download_file(local_filepath, webdav_filepath)
                # add info to new file dictionary
                if VERBOSE:
                    print("Updating new files dict with type info:", webdav_file_dict[webdav_filepath]["type"])
                new_files[local_filename] = {"type": webdav_file_dict[webdav_filepath]["type"], "path": local_filepath, "updated": False}

            else:
                if VERBOSE:
                    print("File exists but getting again:", webdav_filepath)
                # if file exists but md5 different or force redownload
                if webdav_file_dict[webdav_filepath]["local_md5"] != webdav_file_dict[webdav_filepath]["remote_md5"]:
                    if VERBOSE:
                        print("File exists but md5 different:", webdav_file_dict[webdav_filepath]["local_md5"], webdav_file_dict[webdav_filepath]["remote_md5"])
                    local_filepath = webdav_file_dict[webdav_filepath]["local_path"]
                    local_filename = os.path.basename(local_filepath)
                    ...
                    if VERBOSE:
                        print("Renaming existing file from:", local_filepath, "tp", local_filepath_new)
                    os.rename(local_filepath, local_filepath_new)

                    # download new version
                    if VERBOSE:
                        print("Downloading existing file from:", webdav_filepath, "to", local_filepath)
                    download_file(local_filepath, webdav_filepath)

                    # add info to new file dictionary
                    if VERBOSE:
                        print("Updating new files dict with type info:", webdav_file_dict[webdav_filepath]["type"])
                    new_files[local_filename] = {"type": webdav_file_dict[webdav_filepath]["type"], "path": local_filepath, "updated": True}
...
def check_webdav():
    """run the main program: check for new files on Webdav, if so send email"""

    new_files = get_new_updated_webdav_file_dict()
    ...
    if len([s for s in new_files.keys() if not new_files[s]["updated"]]) > 0:
        h += "<h2>The following new files have been uploaded to WebDAV:</h2><ul>\n"
        for filename in new_files.keys():
    ...
    if len([s for s in new_files.keys() if new_files[s]["updated"]]) > 0:
        h += "<h2>The following existing files have been updated on WebDAV:</h2><ul>\n"
        for filename in new_files.keys():
