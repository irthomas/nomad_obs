...
def getMtpConstants(mtpNumber):
    ...
    if mtpNumber == 1:
        ...
        mtpStart = "2025-02-15T14:50:58Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2025-03-15T12:51:54Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20231028_120000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [14, 33, 34, 51, 76, 85, 86, 115, 127, 148, 149, 160, 171, 177, 183, 184, 222,
                                    223, 246, 247, 252, 253, 256, 257, 309, 310, 315, 316, 327, 328, 333]  # input these from the email
        required_dayside_orbits = [2]  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [50, 50, 50, 50]  # GBit # add if required

    elif mtpNumber == 91:
        mtpStart = "2025-03-15T14:49:34Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2025-04-12T12:14:49Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20231028_120000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [13, 18, 24, 47, 85, 86, 97, 127, 128, 171, 172, 173, 183,
                                    184, 203, 220, 224, 256, 257, 259, 290, 333, 334]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [50, 50, 50, 50]  # GBit # add if required

    elif mtpNumber == 92:
        mtpStart = "2025-04-12T14:12:46Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2025-05-10T12:31:41Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20231028_120000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [5, 11, 22, 23, 35, 61, 62, 85, 86, 91, 103, 119, 135, 136,
                                    171, 176, 182, 188, 194, 205, 206, 256, 257, 265]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [50, 50, 50, 50]  # GBit # add if required

    elif mtpNumber == 93:
        mtpStart = "2025-05-10T14:29:42Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2025-06-07T13:04:36Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20250510_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [85, 86, 97, 98, 171, 256, 257, 286, 287, 316, 322, 334, 339]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [50, 50, 50, 50]  # GBit # add if required

    elif mtpNumber == 94:
        mtpStart = "2025-06-07T15:02:35Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2025-07-05T13:12:55Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20250510_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [12, 49, 61, 85, 86, 97, 109, 140, 141, 144, 145, 168, 171, 179, 180, 191,
                                    192, 203, 204, 209, 253, 254, 256, 257, 268, 280, 316, 323, 328]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        ...
        mtpStart = "2025-08-02T14:28:39Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2025-08-30T12:39:56Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20250510_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [14, 15, 21, 49, 50, 53, 54, 66, 85, 86, 88, 89, 95, 107, 123, 130, 171,
                                    178, 191, 209, 245, 252, 253, 256, 257, 292, 293, 298, 299, 322, 323]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [31, 31, 31, 31]  # GBit # add if required

    elif mtpNumber == 97:
        mtpStart = "2025-08-30T14:37:55Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2025-09-27T13:20:11Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20250510_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [2, 3, 5, 6, 17, 18, 26, 27, 85, 86, 154, 155, 162, 163, 168, 169,
                                    171, 177, 178, 201, 207, 208, 219, 220, 234, 249, 256, 257]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [30, 30, 30, 30]  # GBit # add if required

    elif mtpNumber == 98:
        mtpStart = "2025-09-27T15:18:12Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2025-10-25T13:36:16Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20250510_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [48, 54, 68, 69, 70, 71, 77, 85, 86, 91, 97, 103, 109, 110, 139, 140, 171, 201, 202,
                                    216, 217, 222, 223, 234, 235, 246, 247, 256, 257, 286, 287, 303, 304, 316, 335]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [25, 25, 25, 25]  # GBit # add if required

    elif mtpNumber == 99:
        mtpStart = "2025-10-25T15:34:16Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2025-11-22T12:32:25Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20250510_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [18, 28, 29, 78, 79, 85, 86, 97, 110, 123, 124, 124, 125, 166, 171,
                                    185, 211, 222, 252, 253, 255, 256, 261, 280, 298, 316, 317, 328]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [25, 25, 25, 25]  # GBit # add if required
        ...
        mtpStart = "2025-12-20T14:43:45Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2025-12-27T11:49:05Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20250510_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [7, 19, 55, 56, 73]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [25, 0, 0, 0]  # GBit # add if required

    elif mtpNumber == 102:
        mtpStart = "2026-01-21T18:02:36Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2026-02-14T12:01:56Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20250510_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [35, 36, 121, 128, 135, 141, 153, 206, 207, 212, 224, 249, 261, 266]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [0, 25, 25, 25]  # GBit # add if required

    elif mtpNumber == 103:
        mtpStart = "2026-02-14T13:59:53Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2026-03-14T12:32:16Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20250510_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [6, 7, 12, 25, 36, 37, 67, 68, 85, 86, 98, 125, 144, 149,
                                    150, 171, 256, 257, 298, 299, 304, 305, 317, 323, 330]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [20, 25, 25, 25]  # GBit # add if required

    elif mtpNumber == 104:
        mtpStart = "2026-03-14T14:30:20Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2026-04-11T12:55:24Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20250510_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [85, 86, 139, 140, 158, 159, 164, 165, 171, 181, 182, 187, 188,
                                    216, 217, 251, 252, 256, 257, 269, 281, 311, 312, 329, 330]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [25, 25, 25, 25]  # GBit # add if required

    elif mtpNumber == 105:
        mtpStart = "2026-04-11T14:53:20Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2026-05-09T12:30:23Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20250510_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [5, 6, 18, 19, 30, 31, 78, 79, 85, 86, 92, 93, 96, 97, 110, 131, 132, 149,
                                    171, 177, 183, 200, 201, 202, 203, 250, 256, 257, 317, 319, 327]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [43, 43, 43, 43]  # GBit # add if required

    elif mtpNumber == 106:
        mtpStart = "2026-05-09T14:27:56Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2026-06-06T12:19:57Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20250510_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [10, 31, 50, 62, 85, 86, 99, 100, 105, 106, 124, 129, 130, 160, 171,
                                    184, 199, 200, 241, 256, 257, 273, 274, 279, 304, 312, 313, 330]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [31, 31, 31, 31]  # GBit # add if required

    elif mtpNumber == 107:
        mtpStart = "2026-06-06T14:17:51Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2026-07-04T12:46:25Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20260606_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [12, 37, 44, 51, 57, 85, 86, 90, 97, 110, 111, 149,
                                    171, 256, 257, 304, 305, 311, 320, 321, 325, 326]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [44, 44, 44, 44]  # GBit # add if required

    elif mtpNumber == 108:
        mtpStart = "2026-07-04T14:44:28Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2026-08-01T13:15:35Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20260606_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [12, 13, 24, 25, 34, 85, 86, 162, 171, 177, 198, 202, 227,
                                    228, 256, 257, 258, 259, 268, 289, 290, 305, 335, 336]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [37, 37, 37, 37]  # GBit # add if required

    elif mtpNumber == 109:
        mtpStart = "2026-08-01T15:13:31Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2026-08-29T13:36:14Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20260606_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [25, 71, 76, 80, 82, 85, 86, 99, 101, 104, 106, 171, 193, 194, 205, 228,
                                    229, 248, 249, 252, 253, 256, 257, 287, 289, 302, 303, 337, 338]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [50, 50, 50, 50]  # GBit # add if required

    elif mtpNumber == 110:
        mtpStart = "2026-08-29T15:34:28Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2026-09-26T12:28:29Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20260606_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [36, 37, 55, 67, 78, 79, 80, 84, 85, 94, 123, 125, 162, 163, 168, 169,
                                    170, 198, 199, 204, 206, 237, 238, 239, 255, 256, 273, 275, 305]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [44, 44, 44, 44]  # GBit # add if required

    elif mtpNumber == 111:
        mtpStart = "2026-09-26T14:26:21Z"  # EXMGEO_TD2N start time as specified by Bojan or Claudio
        mtpEnd = "2026-10-24T12:39:48Z"  # EXMGEO_TD2N end time as specified by Bojan or Claudio
        copVersion = "20260606_113000"  # desired cop table folder - remember to update if patched
        forbidden_dayside_orbits = [13, 25, 37, 52, 81, 82, 85, 86, 101, 112, 113, 124, 149, 166, 171, 221, 230,
                                    232, 242, 243, 256, 257, 265, 281, 282, 293, 294, 310, 311, 313, 314]  # input these from the email
        required_dayside_orbits = []  # add if LNO needs to operate on certain orbits e.g. joint observations
        occultation_precooling = 440  # seconds required for precooling
        stp_data_volumes = [50, 50, 50, 50]  # GBit # add if required
