...
def makeCopTableDict(channelCode, copTableDict, silent=True):
    ...
    #    subdomainRow = subdomainList[1000]
        for rowIndex, subdomainRow in enumerate(subdomainList):
            ...
            if len(set(binningFactors)) == 1:  # if more than one binning factor in the observation
                ...
                if not silent:
                    print("Binning error row %i" % rowIndex)
                if not silent:
                    print(binningFactors)
                errorFound = True
            ...
            if len(set(accumulations)) == 1:  # if n accumulations is the same for all observations
                ...
                if not silent:
                    print("Accumulation error row %i" % rowIndex)
                if not silent:
                    print(accumulations)
                errorFound = True
            ...
            if len(set(integrationTimes)) == 1:  # if more than one integration time in the observation
                ...
                if not silent:
                    print("Int time error row %i" % rowIndex)
                if not silent:
                    print(integrationTimes)
                errorFound = True

            if nSubdomains > 1:
                if sum(steppingIndices[1:6]) > 0:
                    if not silent:
                        print("Stpping error row %i" % rowIndex)
                    errorFound = True
            ...
            if not calibration:
                ...
                if 650.0 < executionTimeTotal < 1000.0:
                    ...
                    if not silent:
                        print("Exec time error row %i" % rowIndex)
                    errorFound = True
    ...
    elif channelCode in [2]:
        uvisList = copTableDict["uvisList"]
        ...
        for rowIndex, uvisRow in enumerate(uvisList):

            comment = uvisRow[23]
            rhythm = comment.split("Rhythm")[1].replace("=", "").replace("ms", "").strip()
            rhythm = int(rhythm)

            subdomainIndices.append(0)
            aotfOrdersAll.append(0)
            integrationTimesAll.append(uvisRow[13])
            windowHeightAll.append(uvisRow[5])
            rhythmAll.append(rhythm)

    copTableCombinations = {"index": subdomainIndices, "orders": aotfOrdersAll,
                            "integrationTime": integrationTimesAll, "rhythm": rhythmAll, "windowHeight": windowHeightAll}
            ...
            if nFound == len(orders) and nSubdomains == len(orders):
                ...
                if subdomainComment.find("=%sMS" % str(integrationTime)) > -1:
                    ...
                    if subdomainComment.find("NROWS=%s" % str(nRows)) > -1:
                        if not silent:
                            print("Matching row found: %i" % rowIndex)
                        if not silent:
                            print(subdomainRow)
                        return rowIndex
                    else:
    ...
    if channel in ["so", "lno"]:
        ...
        if nSubdomains == 1:  # check for stepping
            ...
            if steppingPointer != "0":
                steppingType, steppingSpeed, steppingCount, steppingValue = findCopRowData(
                    channel, copTableDict, ["steppingParameter", "stepSpeed", "stepCount", "stepValue"], steppingPointer)
...
def getCopRows(observationName, observationDict, copTableDict, copTableCombinationDict, centreDetectorLines, silent=False):
    ...
    if channelCode in [0, 1, 2]:
        channel = {0: "so", 1: "lno", 2: "uvis"}[channelCode]
    else:
    ...
    #            print("Manual mode: COP row %i" %(scienceCopRow))
            ...
            found = [0]
            for indexCop, diffractionOrdersCop, integrationTimeCop, \
                rhythmCop, windowHeightCop in zip(copTableCombinations["index"],
                                                  copTableCombinations["orders"], copTableCombinations["integrationTime"],
                                                  copTableCombinations["rhythm"], copTableCombinations["windowHeight"]):
        ...
        if scienceCopRow < 0:
            print("Error: COP row 1 not found. ", {0: "Orders not found", 1: "Int time not found",
                  2: "Rhythm not found", 3: "Window height not found"}[max(found)])
