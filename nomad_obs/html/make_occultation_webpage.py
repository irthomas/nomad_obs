...
def writeOccultationWebpage(orbit_list, mtpConstants, paths, make_figures=True):
    ...
    htmlHeader = ["Instrument", "Orbit Number", "MTP Number", "Occultation Type", "UTC Start Time", "UTC Transition Time", "UTC End Time", "Duration (s)",
        ...
        for occultationName in occultationNames:
            if occultationName in orbit.keys():
                ...
                if uvisObsType+"CopRows" in orbit["finalOrbitPlan"].keys():
                    uvisDescription = "COP row %i: %s" % (orbit["finalOrbitPlan"][uvisObsType+"CopRows"]["scienceCopRow"],
                                                          orbit["finalOrbitPlan"][uvisObsType+"CopRows"]["copRowDescription"])
                else:
                    uvisDescription = "-"
                ...
                values_dict["50km Local Time (hrs)"].append(getValue("lstTransition"))
    ...
    if make_figures:
        ...
        writeHtmlTable("nomad_mtp%03d_occultation" % mtpNumber, "NOMAD MTP%03d Occultation Observations" % mtpNumber,
                       htmlHeader, htmlRows, paths, linkNameDesc=[linkName, linkDescription], extraComments=extraComments)
