...
from nomad_obs.planning.grazing_requirements import get_max_grazing_altitude

from nomad_obs.config.constants import INITIALISATION_TIME
from nomad_obs.config.constants import OCCULTATION_SEARCH_STEP_SIZE, MAXIMUM_SO_ALTITUDE, SO_TRANSITION_ALTITUDE, \
    MINIMUM_TIME_BETWEEN_OCCULTATIONS, SO_REFERENCE_DURATION
    ...
    for index in range(count):
        ...
        if egress_start - ingress_end < minimum_time_between_occultations:
            ...
            occultation_dict = {"occultationNumber": index+1,
                                "merged": {"utcStart": merged_start_str, "utcEnd": merged_end_str,
                                           "utcMidpoint": merged_midpoint_str, "utcTransition": merged_transition_str,
                                           "etStart": merged_start, "etEnd": merged_end,
                                           "etMidpoint": merged_midpoint, "etTransition": merged_transition,
                                           "lonStart": merged_start_lon, "lonEnd": merged_end_lon,
                                           "lonMidpoint": merged_midpoint_lon, "lonTransition": merged_transition_lon,
                                           "latStart": merged_start_lat, "latEnd": merged_end_lat,
                                           "latMidpoint": merged_midpoint_lat, "latTransition": merged_transition_lat,
                                           "altitudeStart": merged_start_altitude, "altitudeEnd": merged_end_altitude,
                                           "altitudeMidpoint": merged_midpoint_altitude, "altitudeTransition": merged_transition_altitude,
                                           "lstStart": merged_start_lst, "lstEnd": merged_end_lst,
                                           "lstMidpoint": merged_midpoint_lst, "lstTransition": merged_transition_lst,
                                           "obsStart": obs_merged_start, "obsEnd": obs_merged_end, "obsDuration": obs_merged_duration,
                                           "duration": merged_duration,
                                           "etStartAcs": ingress_start_acs}}
        else:
            occultation_dict = {"occultationNumber": index+1,
                                "ingress": {"utcStart": ingress_start_str, "utcEnd": ingress_end_str,
                                            "utcMidpoint": ingress_midpoint_str, "utcTransition": ingress_transition_str,
                                            "etStart": ingress_start, "etEnd": ingress_end,
                                            "etMidpoint": ingress_midpoint, "etTransition": ingress_transition,
                                            "lonStart": ingress_start_lon, "lonEnd": ingress_end_lon,
                                            "lonMidpoint": ingress_midpoint_lon, "lonTransition": ingress_transition_lon,
                                            "latStart": ingress_start_lat, "latEnd": ingress_end_lat,
                                            "latMidpoint": ingress_midpoint_lat, "latTransition": ingress_transition_lat,
                                            "altitudeStart": ingress_start_altitude, "altitudeEnd": ingress_end_altitude,
                                            "altitudeMidpoint": ingress_midpoint_altitude, "altitudeTransition": ingress_transition_altitude,
                                            "lstStart": ingress_start_lst, "lstEnd": ingress_end_lst,
                                            "lstMidpoint": ingress_midpoint_lst, "lstTransition": ingress_transition_lst,
                                            "obsStart": obs_ingress_start, "obsEnd": obs_ingress_end, "obsDuration": obs_ingress_duration,
                                            "duration": ingress_duration,
                                            "etStartAcs": ingress_start_acs},

                                "egress": {"utcStart": egress_start_str, "utcEnd": egress_end_str,
                                           "utcMidpoint": egress_midpoint_str, "utcTransition": egress_transition_str,
                                           "etStart": egress_start, "etEnd": egress_end,
                                           "etMidpoint": egress_midpoint, "etTransition": egress_transition,
                                           "lonStart": egress_start_lon, "lonEnd": egress_end_lon,
                                           "lonMidpoint": egress_midpoint_lon, "lonTransition": egress_transition_lon,
                                           "latStart": egress_start_lat, "latEnd": egress_end_lat,
                                           "latMidpoint": egress_midpoint_lat, "latTransition": egress_transition_lat,
                                           "altitudeStart": egress_start_altitude, "altitudeEnd": egress_end_altitude,
                                           "altitudeMidpoint": egress_midpoint_altitude, "altitudeTransition": egress_transition_altitude,
                                           "lstStart": egress_start_lst, "lstEnd": egress_end_lst,
                                           "lstMidpoint": egress_midpoint_lst, "lstTransition": egress_transition_lst,
...
def findGrazingOccultations(orbit_list, mtp_constants):
    """find all grazing occultations in MTP, add to orbit list.
    Note that this finds all NOMAD grazings below the maximum altitude for both channels, further geometry selection is done later"""
    precooling_time = mtp_constants["occultation_precooling"]

    maximum_grazing_altitude = get_max_grazing_altitude(mtp_constants)
                ...
                if 0.0 < minimum_altitude < maximum_grazing_altitude:  # if grazing and below
                    ...
                    occultation_dict = {"occultationNumber": grazing_index,
                                        "grazing": {"utcStart": grazing_start_str, "utcEnd": grazing_end_str,
                                                    "utcMidpoint": grazing_midpoint_str, "utcTransition": "-",
                                                    "etStart": grazing_start, "etEnd": grazing_end,
                                                    "etMidpoint": grazing_midpoint, "etTransition": "-",
                                                    "lonStart": grazing_start_lon, "lonEnd": grazing_end_lon,
                                                    "lonMidpoint": grazing_midpoint_lon, "lonTransition": "-",
                                                    "latStart": grazing_start_lat, "latEnd": grazing_end_lat,
                                                    "latMidpoint": grazing_midpoint_lat, "latTransition": "-",
                                                    "altitudeStart": grazing_start_altitude, "altitudeEnd": grazing_end_altitude,
                                                    "altitudeMidpoint": grazing_midpoint_altitude, "altitudeTransition": "-",
                                                    "lstStart": grazing_start_lst, "lstEnd": grazing_end_lst,
                                                    "lstMidpoint": grazing_midpoint_lst, "lstTransition": "-",
