...
from nomad_obs.planning.grazing_requirements import check_grazing_geometry, get_max_grazing_altitude
from nomad_obs.config.constants import NADIR_SEARCH_STEP_SIZE, MAXIMUM_SEARCH_INCIDENCE_ANGLE, OCCULTATION_SEARCH_STEP_SIZE


def regionsOfInterestNadir(orbit_list, mtp_constants, regions_of_interest, observationCycles, silent=True):
            ...
            for regionOfInterest in regions_of_interest:
                ...
                if np.any(matches):
                    ...
                    if incidence_angles[i] < MAXIMUM_SEARCH_INCIDENCE_ANGLE:  # check if solar angle too low
                        ...
                        if not silent:
                            print("Match found on orbit %i but incidence angle %0.1f is above %0.0f" %
                                  (orbit["orbitNumber"], incidence_angles[i], MAXIMUM_SEARCH_INCIDENCE_ANGLE))
    return orbit_list


def regionsOfInterestOccultation(orbit_list, mtp_constants, regions_of_interest, observationCycles, silent=True):
    """Check for occultation observations near regions of interest.
    Note that specific grazing geometry requirements of each channel are not considered here, only the maximum altitude for both channels"""
    ...
    for orbit in orbit_list:
        if "daysideRegions" in orbit.keys():
            ...
            for region in orbit["daysideRegions"]:
                ...
                if not silent:
                    print("Match found: %s, orbit %i, incidence angle %0.1f at %s" %
                          (region["name"], orbit["orbitNumber"], region["incidenceAngle"], region["utc"]))
                ...
                if region["priority"] < priority:  # if region has higher priority, write to orbit list
                    ...
                    print("Matching region of interest found (%s), but superseded by higher priority region (%s)" %
                          (region["name"], orbit["dayside"]["observationName"]))
        ...
        if "occultationRegions" in orbit.keys():
            ...
            for region in orbit["occultationRegions"]:
                ...
                if region["priority"] < priority:  # if region has higher priority, write to orbit list
                    ...
                    print("Matching region of interest found (%s), but superseded by higher priority region (%s)" %
                          (region["name"], orbit[matchingOccultationType]["observationName"]))
