...
def calculate_best_inttimes(base_inttime, rhythm, nsubdomains, windowheight, order_combinations=[[]], fix_inttime=False):
    ...
    for inttime in possible_inttimes:
        ...
        if not fix_inttime:
            print("int time %i ms: exec time %i, lost time %0.1f ms (%i accs)" %
                  (inttime / 1000, (execution_time * nsubdomains) / 1000, lost_time, accumulations))
...
def checkExecTime(exec_time_microsecs, rhythm, silent=False):
    ...
    if exec_time_microsecs < min_exec_time:
        if not silent:
            print(f"Error: execution time too small (exec_time={exec_time_microsecs}, min_exec_time={min_exec_time}, rhythm={rhythm}")
#        stop()
        return 0
    else:
        if exec_time_microsecs > max_exec_time:
            if not silent:
                print(f"Error: Execution time too large (exec_time={exec_time_microsecs}, max_exec_time={max_exec_time}, rhythm={rhythm}")
#            stop()
            return 0
        else:
            return 1
...
cases = [
    ...
    # {"it": 200000, "rhy": 15000000, "nsub": 3, "wh": 136},
    {"it": 200000, "rhy": 15000000, "nsub": 6, "wh": 136},
]
