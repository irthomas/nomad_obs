...
old_dir_name = "20250510_113000"  # latest existing cop tables
temp_dir_name = datetime.now().strftime("%Y%m%d_%H%M%S") + "_new"  # timestamped temporary directory
new_dir_name = "mtp107_proposed"  # final directory

logger_level = "DEBUG"
logger_name = os.path.join("cop_patching", "log.txt")


# only if new fixed rows will be needed, otherwise leave empty
lno_centre_rows = [148]  # FIXME: only for special 136 rows, should be 152 normally!
so_centre_rows = []
...
# don't comment out science or subdomain
available_cop_rows = {
    "lno": {
        "fixed": [*range(255, 258)],
        # "stepping": [*range(73, 258)],
        "science": [*range(4039, 4098)],
        "sub_domain": [*range(4086, 4096)]
    },

    "so": {
        # "fixed":[*range(111, 258)],
        # "stepping": [*range(27, 258)],
        "science": [*range(4081, 4098)],
        "sub_domain": [*range(3252, 4098)]
    }
...
def parse_subd_comment(comment):
    ...
    regex = re.compile(
        r"(?:ORDERS (.*) -- (\S*)_(\d*)ROWS_(\d*)SECS_(\d*)SUBDS.*EXECTIME=(\d*)MS|(\S*)_(\d*)-(\d*)-(\d*)ORDERS_(\d*)ROWS_(\d*)SECS_(\d*)SUBDS.*EXECTIME=(\d*)MS|EMPTY ROW)")
    ...
    if len(match) == 6:
        ...
        match = try_int(match)
        parsed_dict = {"name": match[0], "start": match[1], "step": match[2], "stop": match[3],
                       "rows": match[4], "rhythm": match[5], "n_orders": match[6], "exec_time": match[7]}
...
def new_science_rows(channel, new_obs_dict, old_sci_dir, sci_dir_out):
    ...
    for d in sci_dict_list:
        ...
        line = "%i,%i,%i,%i,%i,%i,%i,%i # %s" % (d["degf"], d["dvaf"], d["sbsf"], d["aotfPointer"], d["steppingPointer"],
                                                 d["accumulationCount"], d["binningFactor"], d["integrationTime"], comment)
...
if __name__ == "__main__":
    ...
    if MAKE_TABLES:
        ...
        if "so_science" in OBS_TO_UPDATE:
            ...
            # then read in old directory, merge new lines and output to new dir
            # first line to start replacing rows (as given in notepad++)
            so_science_used_rows, so_science_unused_rows = replace_lines("so", "science", rows_to_replace, existing_dir, new_lines_dir, merged_dir)
            ...
            # then read in old directory, merge new lines and output to new dir
            so_sub_domain_used_rows, so_sub_domain_unused_rows = replace_lines(
                "so", "sub_domain", rows_to_replace, existing_dir, new_lines_dir, merged_dir)  # first line to start replacing rows (as given in notepad++)
        ...
        if "so_fullscan_stepping" in OBS_TO_UPDATE:
            ...
            # then read in old directory, merge new lines and output to new dir
            # first line to start replacing rows (as given in notepad++)
            replace_lines("so", "stepping", rows_to_replace, existing_dir, new_lines_dir, merged_dir)
        ...
        if "so_fullscan_science" in OBS_TO_UPDATE:
            ...
            # then read in old directory, merge new lines and output to new dir
            # first line to start replacing rows (as given in notepad++)
            so_science_used_rows, so_science_unused_rows = replace_lines("so", "science", so_science_unused_rows, existing_dir, new_lines_dir, merged_dir)
            ...
            new_subdomain_fullscan_rows("so", stepping_dir_in, sci_dir_in, subd_dir_out,
                                        science_rows_to_add=so_science_used_rows)  # write new lines to temp dir
            # then read in old directory, merge new lines and output to new dir
            replace_lines("so", "sub_domain", so_sub_domain_unused_rows, existing_dir, new_lines_dir,
                          merged_dir)  # first line to start replacing rows (as given in notepad++)
        ...
        if "lno_fixed" in OBS_TO_UPDATE:
            ...
            # then read in old directory, merge new lines and output to new dir
            # first line to start replacing rows (as given in notepad++)
            replace_lines("lno", "fixed", rows_to_replace, existing_dir, new_lines_dir, merged_dir)
        ...
        if "lno_science" in OBS_TO_UPDATE:
            ...
            # then read in old directory, merge new lines and output to new dir
            # first line to start replacing rows (as given in notepad++)
            lno_science_used_rows, lno_science_unused_rows = replace_lines("lno", "science", rows_to_replace, existing_dir, new_lines_dir, merged_dir)
            ...
            # then read in old directory, merge new lines and output to new dir
            lno_sub_domain_used_rows, lno_sub_domain_unused_rows = replace_lines(
                "lno", "sub_domain", rows_to_replace, existing_dir, new_lines_dir, merged_dir)  # first line to start replacing rows (as given in notepad++)
        ...
        if "lno_fullscan_stepping" in OBS_TO_UPDATE:
            ...
            # then read in old directory, merge new lines and output to new dir
            replace_lines("lno", "stepping", rows_to_replace, existing_dir, new_lines_dir,
                          merged_dir)  # first line to start replacing rows (as given in notepad++)
        ...
        if "lno_fullscan_science" in OBS_TO_UPDATE:
            ...
            # then read in old directory, merge new lines and output to new dir
            # first line to start replacing rows (as given in notepad++)
            lno_science_used_rows, lno_science_unused_rows = replace_lines("lno", "science", lno_science_unused_rows, existing_dir, new_lines_dir, merged_dir)
            ...
            new_subdomain_fullscan_rows("lno", stepping_dir_in, sci_dir_in, subd_dir_out,
                                        science_rows_to_add=lno_science_used_rows)  # write new lines to temp dir
            # then read in old directory, merge new lines and output to new dir
            replace_lines("lno", "sub_domain", lno_sub_domain_unused_rows, existing_dir, new_lines_dir,
                          merged_dir)  # first line to start replacing rows (as given in notepad++)
    ...
    for channel in ["so", "lno"]:
        ...
        for sub_dict in sub_dict_list:  # [0:10]:
            ...
            if "orders" in subd_parsed_dict.keys():  # if normal obs
                ...
                text = "%s_FULLSCAN_FAST_%i-%i-%iORDERS_%iROWS_%iSECS_%iSUBDS -- EXECTIME=%iMS" % (
                    comment_type, start, step, stop, d_rows, rhythm, n_orders, et)
...
# %%
# open old and new files in notepad++
npp_path = r"C:/Program Files/Notepad++/notepad++.exe"

# select one channel and one table type to open in notepad++. Use compare plugin to check changes
channel = "lno"
# channel = "so"
table_type = "fixed"
# table_type = "science"
# table_type = "sub_domain"

old_path = make_cop_path(old_dir_name, channel, table_type)
new_path = make_cop_path(new_dir_name, channel, table_type)

if os.path.exists(npp_path):
    subprocess.Popen("%s %s" % (npp_path, old_path))
    subprocess.Popen("%s %s" % (npp_path, new_path))
