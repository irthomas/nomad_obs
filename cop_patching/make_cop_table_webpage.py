...
WRITE COP TABLES TO WEBPAGE
WORKING DIRECTORY MUST BE NOMAD_OBS
"""
...
#    subdomainRow = subdomainList[1000]
    for rowIndex, subdomainRow in enumerate(subdomainList):
        ...
        if len(set(sbsfs)) == 1:  # if more than one integration time in the observation
            ...
            if not silent:
                print("Sbsf error row %i" % rowIndex)
            if not silent:
                print(sbsf)
            errorFound = True
    ...
    copTableCombinations = {"index": subdomainIndices, "orders": aotfOrdersAll,
                            "integrationTime": integrationTimesAll, "rhythm": rhythmAll, "windowHeight": windowHeightAll, "sbsf": sbsfAll}
...
for channelCode in [0, 1]:
    ...
    for numberOfOrders in tables.keys():
        if numberOfOrders == 1:
            h += r"<h2>%s diffraction order</h2>" % numberOfOrders + "\n"
        else:
            h += r"<h2>%s diffraction orders</h2>" % numberOfOrders + "\n"
        h += tables[numberOfOrders]
        h += "<br><br>" + "\n"
    h += "Made with nomad_obs.cop_patching.make_cop_table_webpage.py" + "\n"
